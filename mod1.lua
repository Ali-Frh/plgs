...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '😐گروه از مدیریت من خارج شد'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local kicked_num = "تعداد کاربران اخراجی: "..result.kicked_count.."\n"
local channel_id = "ایدی😐: "..result.peer_id.."\n"
if result.username then
	channel_username = "یوزر نیم😐: @"..result.username
else
	channel_username = ""
end
...
--vardump(result)
local text = "اخراج شدگان سوپرگروه 😐 : "..cb_extra.receiver.."\n\n"
local i = 1
for k,v in pairsByKeys(result) do
if not v.print_name then
	name = " "
...
local function modlist(msg)
  ...
  if next(data[tostring(msg.to.id)]['moderators']) == nil then
    return '👎 moderator in this group.'
  end
  local i = 1
...
local function run(msg, matches)
	...
	local data = load_data(_config.moderation.data)
		...
		if matches[1] == 'newlink' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
					send_large_msg(receiver, '😐عامو من سازنده ی گروه نیستم')
		...
		if matches[1] == 'setlink' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
			return 'لینک جدید گروه رو بفرست لطفا😐'
		end
		...
		if matches[1] == 'link' then
			...
			if not group_link then
				return ":|یا لینک جدید بساز یا اگر من سازنده ی گروه نیستم لینک رو به من بده\n /setlink \n /newlink"
			end
			savelog(msg.to.id, name_log.." ["..msg.from.id.."] requested group link ["..group_link.."]")
			return "⚪️لینک گروه"..msg.to.title.. "\n🇮🇷" ..group_link
		end
		...
		if matches[1] == 'help' and not is_owner(msg) then
			text = "Message /superhelp to @ThinkTg in private for SuperGroup help"
